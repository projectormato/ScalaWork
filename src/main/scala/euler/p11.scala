object p11 extends App {
  var result:Long= 1; var sum:Long = 0;
  val input = List(
  ...
  for (i <- 0 to 19){
    for(j <- 0 to 16){
      sum = input(i).slice(j, j+4).product //48477312
      result = List(result, sum).max
      sum = List(input(j)(i), input(j+1)(i), input(j+2)(i), input(j+3)(i)).product //51267216
      result = List(result, sum).max
    }
  }
  // ななめ
  for (i <- 0 to 16){
    for(j <- 0 to 16){
      sum = List(input(i)(j), input(i+1)(j+1), input(i+2)(j+2), input(i+3)(j+3)).product
      result = List(result, sum).max
      sum = List(input(19-i)(j), input(19-i-1)(j+1), input(19-i-2)(j+2), input(19-i-3)(j+3)).product //70600674
      result = List(result, sum).max
    }
  }
  println(result)
